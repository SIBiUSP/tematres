        ...
        if (isset ($_GET["task"])) {
            switch ($_GET["task"]) {
                ...
                case 'letter':
                    ...
                    $htmlTerm   = data2html4Letter($dataTerm,array("vocab_code"=>$v,"div_title"=>"Termos com a letra "));
                    $task       = 'letter';
                break;
                //ultimos términos
                case 'fetchLast':
                    $dataTerm   = getURLdata($URL_BASE.'?task=fetchLast');
                    $htmlTerm   = data2html4LastTerms($dataTerm,array("vocab_code"=>$v,"div_title"=>"Últimas alterações"));
                    $task       = 'fetchLast';
                break;
                //búsqueda  == search
                case 'search':
