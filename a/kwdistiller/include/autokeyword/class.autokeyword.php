...
*/
	...
	{
		$keywords = $this->parse_words($params).$this->parse_2words().$this->parse_3words();
	...
	function parse_words($param)
	{
		//eval lang var
		$stop_words_lang=(in_array($param["lang"],array('en','it','es','pt','fr'))) ? $param["lang"] : 'en';

		include("stopwords-$stop_words_lang.php");
		//list of commonly used words

		$common = $array_stop_words;
	...
	function parse_2words($param)
	{
		...
		for ($i=0; $i < count($x)-1; $i++) {
			...
			if( (mb_strlen(trim($x[$i])) >= $this->word2WordPhraseLengthMin ) 
				&&  !in_array(trim($x[$i]), $common)  
				&&  !in_array(trim($x[$i+1]), $common)  
